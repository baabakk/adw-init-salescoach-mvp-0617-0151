...
> Parsed at 2026-06-16T17:26:17.998Z
> Source: File-based
> Risk Score: 21/100
...
- **AC-01** [P1, testable]: Pre-call conversational intake completes Q1-Q3 super-clarifying questions with LLM-driven data extraction and contextual follow-ups until all required fields are filled
- **AC-02** [P1, testable]: Framework selection returns SPICED (primary) + MEDDPICC (secondary) and proceeds to guidebook-derived question packs for missing fields
- **AC-03** [P1, testable]: Call Reasoning (CR1-CR5), Persona (P1-P5), and Demographics (D1-D5) stages complete sequentially, each gated by a fire-once completion event
- **AC-04** [P1, testable]: Call plan is generated with: objective, 3 priorities, 2 risks, target next step, and unknowns list
- **AC-05** [P1, testable]: Live transcription displays with speaker diarization (rep vs. prospect) at sub-500ms latency via AssemblyAI streaming
- **AC-06** [P1, testable]: Coaching cues appear on-screen within 500ms of trigger detection, throttled to max 1 per 30 seconds
- **AC-07** [P1, testable]: Post-call summary, scorecard, and lessons are generated within 10 seconds of call end
- **AC-08** [P1, testable]: All extracted data carries confidence tags (confirmed, inferred, assumed)
- **AC-09** [P1, testable]: WebSocket reconnects automatically (max 5 attempts, 3s delay) without losing call state
- **AC-10** [P1, testable]: LLM provider failover switches between Cerebras and OpenAI transparently on provider failure
- **AC-11** [P1, testable]: UI renders with OnPush change detection — no visible jank during live call with concurrent transcript + coaching cue updates
- **AC-12** [P1, testable]: All event-driven communication flows through EventBus — no direct handler-to-handler calls

## Dependencies

### External

- ****AssemblyAI v3 Streaming API** — real-time transcription with `format_turns=true` and `ForceEndpoint` message support** (service, risk: medium)
- ****Cerebras API** — primary LLM provider for fast inference (sub-second response for extraction and coaching)** (service, risk: medium)
- ****OpenAI API** — fallback LLM provider when Cerebras is unavailable or degraded** (service, risk: medium)
- ****Voice Engine Proxy** (prototypedemos.com) — proxies TTS requests to ElevenLabs/FishAudio** (service, risk: medium)
- ****MongoDB** — persistent storage for users, campaigns, contacts, call sessions, transcripts, and call plans** (service, risk: medium)
- ****SPICED + MEDDPICC Guidebook Data** — loaded from JSON into GuidebookStore at startup; defines dimensions, required fields, and question banks** (service, risk: medium)
- ****Pre-Call Questions Schema** (`precall-questions-schema.json`) — defines initial questions, field metadata, and extraction rules for Q1-Q3** (service, risk: medium)
- ****Fuse Admin Template** — Angular Material + TailwindCSS design system providing layout, navigation, and component library** (service, risk: medium)
- ****Docker + Nginx** — containerized deployment with reverse proxy for WebSocket upgrade support** (service, risk: medium)
- ****AWS ECS** — production hosting (ECR for images, Secrets Manager for API keys, ALB for traffic)** (service, risk: medium)
...
**Factors:**
- 10 dependencies identified (elevated risk)

## Delivery Intent

- **Scope:** mvp
- **Timeline:** - **Phase 0 — Foundation & Infrastructure:** COMPLETE
...
- **Budget:** - Engineering: 1 founder-developer, full-time (~5 weeks remaining to MVP)
...
**Gaps:**
- No LLM available — contract built from file parsing only. Requirements and edge cases not extracted.
